...
import com.github.jgzl.bsf.core.base.Ref;
...
public class ThreadPool {
     ...
     * @param <T>
     */
    public <T> void parallelFor(String taskName,int parallelCount, List<T> array, final com.github.jgzl.bsf.core.base.Callable.Action1<T> action) {
        checkHealth();
     ...
     * @param <T>
     */
    public <T> void parallelFor2(String taskName,int parallelCount, List<T> array, final com.github.jgzl.bsf.core.base.Callable.Action1<T> action){
        checkHealth();
        threadMonitor.hook().run(taskName, ()-> {
            int parallelCount2 = parallelCount;
            if(parallelCount2>array.size()){
    ...
    static class SystemThreadPoolFactory implements ThreadFactory {
        ...
        public Thread newThread(Runnable r) {
            ...
            if(handler==null||!(handler instanceof DefaultBsfThreadPoolUncaughtExceptionHandler)) {
                t.setUncaughtExceptionHandler(new DefaultBsfThreadPoolUncaughtExceptionHandler(handler));
            }
            //后台线程模式
            t.setDaemon(true);
            return t;
    ...
    public static class DefaultBsfThreadPoolUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
        private Thread.UncaughtExceptionHandler lastUncaughtExceptionHandler = null;
        public DefaultBsfThreadPoolUncaughtExceptionHandler(Thread.UncaughtExceptionHandler lastUncaughtExceptionHandler){
        ...
        public void uncaughtException(Thread t, Throwable e){
            try {
                if(e!=null) {
                    LogUtils.error(DefaultBsfThreadPoolUncaughtExceptionHandler.class,CoreProperties.Project,"【警告】bsf TheadPool未捕获错误",e);
                }
            }catch (Exception e2){}
