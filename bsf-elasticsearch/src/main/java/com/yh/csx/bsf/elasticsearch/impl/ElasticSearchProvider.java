...
public class ElasticSearchProvider implements AutoCloseable {
	 ...
	 * @param index         索引名
	 * @param type          类型
	 * @param mappingJson mapping规则
	 */
	public void createIndex(String indexName, String type, String mappingJson) {
		ElasticSearchMonitor.hook().run("createIndex", () -> {
			getClient().admin().indices().prepareCreate(indexName).addMapping(type, mappingJson, XContentType.JSON).get();
		});
	}
     ...
     * @param mappingJson mapping规则
	 */
	public void putMapping(String indexName, String type, String mappingJson) {
		ElasticSearchMonitor.hook().run("putMapping", () -> {
    ...
    public void insertData(String index, String type, String json) {
        ElasticSearchMonitor.hook().run("insertData", () -> {
            getClient().prepareIndex(index, type).setSource(json, XContentType.JSON).get();
        });
    }
    ...
    public void insertData(String index, String type, String _id, String json) {
        ElasticSearchMonitor.hook().run("insertData", () -> {
            getClient().prepareIndex(index, type).setId(_id).setSource(json, XContentType.JSON).get();
        });
    }
    /**
     * 	 功能描述：插入数据
    ...
    public void updateData(String index, String type, String _id, String json) throws Exception {
        ElasticSearchMonitor.hook().run("updateData", () -> {
            try {
                UpdateRequest updateRequest = new UpdateRequest(index, type, _id).doc(json,XContentType.JSON);
    ...
    public void bulkInsertData(String index, String type, Map<String, String> data) {
        ElasticSearchMonitor.hook().run("bulkInsertData", () -> {
            ...
            data.forEach((param1, param2) -> {
                bulkRequest.add(getClient().prepareIndex(index, type, param1).setSource(param2,XContentType.JSON)
                );
            });
            bulkRequest.get();
        });
    ...
    public void bulkInsertData(String index, String type, List<String> jsonList) {
        ElasticSearchMonitor.hook().run("bulkInsertData", () -> {
            ...
            jsonList.forEach(item -> {
                bulkRequest.add(getClient().prepareIndex(index, type).setSource(item, XContentType.JSON)
                );
            });
            bulkRequest.get();
        });
